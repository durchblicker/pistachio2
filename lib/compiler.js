...
function parse(text, options, callback) {
  ...
  text = text.replace(parse.item.partialJSON, function(match, jsonName, filename) {
    parts.push(new PartialJSON(jsonName, filename));
    return parse.marker(parts.length - 1);
  });
  ...
  text = text.replace(parse.item.partialEx, function(match, filename, params) {
    parts.push(new Partial(filename, params));
    return parse.marker(parts.length - 1);
  });
...
parse.item.partialEx = /\{\{\s*\>\s*(\S*?) (.*?)\}\}/g;
parse.item.partialJSON = /\{\{\s*\>json\s*(\S*?)\s*(\S*?)\s*\}\}/g;
...
function Partial(filename, params) {
  Template.call(this, true);
  if(filename) {
...
Partial.prototype.parse = function(options, callback) {
  ...
  var templatePath = this.fullpath;
  if(templatePath.indexOf(options.standardBasePath+'/_split/templates')==0){
    templatePath = templatePath.replace(new RegExp('^'+options.standardBasePath+'/_split','g'),options.ruleBasePath+'/test/'+options.testId);
  }
  if(typeof options.testBasePath != 'undefined' && (!templatePath.indexOf(options.standardBasePath+'/test')==0 && !templatePath.indexOf(options.standardBasePath+'/_split')==0) ){
    templatePath=templatePath.replace(new RegExp('^'+options.standardBasePath),options.testBasePath);
    that.fullpath = templatePath;
    that.filename = templatePath;
  }
  fs.stat(templatePath, function (err, stat) {
    if(err){
      templatePath = templatePath.replace(options.replacePathPart,'');
      if(options.testId != ''){
        templatePath = templatePath.replace('/test/'+options.testId,'');
      }
      that.fullpath = templatePath;
      that.filename = templatePath;
    }
...
Partial.prototype.code = function(lvl) {
  ...
  //if(this.params){
    code.push('(function(params){');
  //}

  //if(this.params){
    code.push('params = params || {}; return $join([');
  //} else {
  //  code.push('$join([');
  //}
...
function PartialJSON(jsonName, filename) {
  Template.call(this, true);
  if(filename) {
    Object.defineProperty(this, 'filename', {
      value: filename
   ...
   if(jsonName) {
    Object.defineProperty(this, 'jsonName', {
      value: jsonName
    });
  }
}
PartialJSON.prototype = new Template();
PartialJSON.prototype.parse = function(options, callback) {
  if(('function' === typeof options) && (!callback)) {
    callback = options;
    options = {};
  }
  ...
  var jsonPath = this.fullpath;
  if(jsonPath.indexOf(options.standardBasePath+'/_split/templates')==0){
    jsonPath = jsonPath.replace(new RegExp('^'+options.standardBasePath+'/_split','g'),options.ruleBasePath+'/test/'+options.testId);
  }
  if(typeof options.testBasePath != 'undefined' && (!jsonPath.indexOf(options.standardBasePath+'/test')==0 && !jsonPath.indexOf(options.standardBasePath+'/_split')==0) ){
    jsonPath=jsonPath.replace(new RegExp('^'+options.standardBasePath),options.testBasePath);
    that.fullpath = jsonPath;
    that.filename = jsonPath;
  }
  fs.stat(jsonPath, function (err, stat) {
    if(err){
      jsonPath = jsonPath.replace(options.replacePathPart,'');
      if(options.testId != ''){
        jsonPath = jsonPath.replace('/test/'+options.testId,'');
      }
      that.fullpath = jsonPath;
      that.filename = jsonPath;
    }
    ...
    fs.readFile(jsonPath, 'utf-8', function(err, text) {
      ...
      Object.defineProperty(that, 'json', {
        value: text
      });
      return callback();
    });
...
PartialJSON.prototype.partials = function() {
  var result = [this.fullpath];
  // don't search for partials here - no partials in json
  return result;
};
PartialJSON.prototype.code = function(lvl) {

  var code = [];

  code.push('((function() { root.json = root.json || {};');

  code.push('root.json[\'' + this.jsonName + '\']=');
  code.push(JSON.stringify(this.json));
  code.push(';');
...
PartialJSON.prototype.toString = function() {
  return ['PartialJSON(', this.fullpath || this.filename, ')'].join('');
};
...
Globbed.prototype.parse = function(options, callback) {
  ...
  glob(this.glob, { cwd:base }, function(err, files) {
    ...
    async.each(that.parts, function(part, callback) {
...
function parseTemplate(template, options, callback) {
  ...
  if(typeof options.testBasePath != 'undefined' && template.indexOf(options.standardBasePath)==0 && (!template.indexOf(options.standardBasePath+'/test')==0 && !template.indexOf(options.standardBasePath+'/_split')==0) ){
    template=template.replace(new RegExp('^'+options.standardBasePath),options.testBasePath);
  }
  fs.stat(template, function(err, stat) {
    if(err && template.indexOf(options.standardBasePath)==0 && typeof options.testBasePath!= 'undefined'){
      template=template.replace(new RegExp('^'+options.testBasePath),options.standardBasePath);
      parseFile(template, options, callback);
    }else{
